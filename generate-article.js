  ...
  animals: [
    {
      ...
      prompt: "Расскажи, как ласточки, летая низко, предсказывают дождь, и дай научное объяснение этому явлению. Укажи, почему их поведение меняется в зависимости от атмосферного давления."
    },
    {
      title: "Погода и животный мир",
      prompt: "Объясни, как поведение муравьев и пчел связано с приближающейся непогодой. Например, почему они спешат укрыться перед дождем, и как их инстинкты помогают им выживать в меняющихся условиях."
    },
    {
      title: "Погода и животный мир",
      prompt: "Поведай о том, как пауки и их паутина могут служить индикаторами погоды. Объясни, почему перед дождем пауки часто не плетут свои сети."
    }
  ],
  astronomy: [
    {
...
async function generateArticle(weatherData, timeOfDayRu) {
  ...
  let topicIndex = 0;
  if (fs.existsSync('topic-index.json')) {
    const savedIndex = JSON.parse(fs.readFileSync('topic-index.json'));
    topicIndex = savedIndex.index;
  }
  
  const totalTopics = topicData.mythology.length;
  const mythologyTopic = topicData.mythology[topicIndex % totalTopics];
  const animalsTopic = topicData.animals[topicIndex % totalTopics];
  const astronomyTopic = topicData.astronomy[topicIndex % totalTopics];

  // Сохраняем индекс для следующего запуска
  fs.writeFileSync('topic-index.json', JSON.stringify({ index: (topicIndex + 1) % totalTopics }), 'utf-8');
...
— ${mythologyTopic.title}: ${mythologyTopic.prompt}
— ${animalsTopic.title}: ${animalsTopic.prompt}
— ${astronomyTopic.title}: ${astronomyTopic.prompt}
