...
— История дня: Хроники Тото: Напиши увлекательный рассказ объёмом около 150-200 слов. В нем должен быть керн-терьер Тото. У Тото есть хозяин. В истории должна быть завязка, кульминация и непредсказуемый финал, который является завязкой для следующей истории. Используй слова 'собака', 'пёс', 'друг человека'. Не используй 'пёсик'. Сюжет должен быть связан с погодным явлением.
...
`;
  ...
  try {
    ...
    // NEW: Сохранение последней истории о Тото для возможного продолжения
    const totoSectionHeader = "История дня: Хроники Тото";
    const startIndex = generatedText.indexOf(totoSectionHeader);
    if (startIndex !== -1) {
        let endIndex = generatedText.indexOf("\n\n", startIndex + totoSectionHeader.length);
        if (endIndex === -1) {
            endIndex = generatedText.length;
        }
        const totoStory = generatedText.substring(startIndex + totoSectionHeader.length, endIndex).trim();
        fs.writeFileSync('toto_story_state.json', JSON.stringify(totoStory), 'utf-8');
    }
