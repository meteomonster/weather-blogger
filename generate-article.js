...
async function generateArticle(weatherData, timeOfDay) {
  ...
  const feelsLikeSignificant = weatherData.apparent_temperature_min.some((t_min_feels, i) => {
      const t_max_feels = weatherData.apparent_temperature_max[i];
      const t_min_real = weatherData.temperature_2m_min[i];
      const t_max_real = weatherData.temperature_2m_max[i];
      return Math.abs(t_min_feels - t_min_real) > 1 || Math.abs(t_max_feels - t_max_real) > 1;
  });

  const feelsLikeDataString = feelsLikeSignificant
...
1. ИСПОЛЬЗУЙ ТОЛЬКО ПРЕДОСТАВЛЕННЫЕ ДАННЫЕ: Это самое важное правило. Не придумывай и не изменяй цифры, даты или исторические факты. Вся информация для статьи находится в блоке "НЕОБРАБОТАННЫЕ ДАННЫЕ". Будь точным журналистом, а не писателем-фантастом.
2. НИКАКОГО MARKDOWN: Не используй символы ##, **, * или любые другие. Только чистый текст.
3. ТОЛЬКО РЕАЛЬНЫЕ ДАТЫ: Вместо "1-й день", "2-й день" используй настоящие даты.
4. ПОДЗАГОЛОВКИ: Каждый подзаголовок (например, "Детальный прогноз по дням") должен быть на отдельной строке. После подзаголовка ОБЯЗАТЕЛЬНО должна быть одна пустая строка.
5. БЕЗУПРЕЧНАЯ ГРАМОТНОСТЬ: Текст должен быть написан на безупречном русском языке. Обрати особое внимание на правильное использование предлогов (например, "во вторник", а не "в вторник"; "со среды", а не "с среды").
...
${dates[0]} — подробно утро, день, вечер, ночь: температура, облачность, осадки, ветер. Упоминай температуру по ощущению только если она заметно отличается от фактической (разница > 1°C).
...
Мини-рубрика “Сегодня в истории”: Используй только этот текст: ${historicalRecord}.
...
- Температура воздуха (мин/макс): ${weatherData.temperature_2m_min.map((t, i) => `${dates[i]}: ${t.toFixed(1)}°C...${weatherData.temperature_2m_max[i].toFixed(1)}°C`).join("; ")}${feelsLikeDataString}
