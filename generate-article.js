 ...
 * — Получает исторические рекорды именно на сегодняшнюю календарную дату (Open-Meteo Archive)
 * — Тянет глобальные события из USGS (землетрясения) и NHC (тропические циклоны)
...
async function getWeatherData() {
  ...
  try {
    ...
    for (const day of forecastDays) {
      ...
      // При желании можно добавить рассвет/закат из астрономического API. Сейчас — пусто (не выдумываем).
...
async function getGlobalEvents() {
  ...
  try {
    ...
    // Структура NHC может меняться; ищем массивы штормов в корне/потомках
    const candidates = [];
    ...
    const toKmh = (value, unit) => {
      ...
      if (u.includes("kt") || u.includes("knot")) return Math.round(v * 1.852);  // узлы → км/ч
      if (u.includes("mph")) return Math.round(v * 1.60934);                     // миль/ч → км/ч
      if (u.includes("km"))  return Math.round(v);                                // уже км/ч
      // если метка неизвестна — в NHC чаще узлы
...
async function generateArticle(weatherData, timeOfDayRu) {
  ...
  const dataPayload = {
    ...
    globalEvents: {
      ...
      hurricanes: global.hurricanes
    }
  };

  // Жёсткие правила для раздела "Экстремальные события"
...
Твоя задача: Подготовить ${timeOfDayRu} выпуск с местной и глобальной картиной погоды.
...
4. Подзаголовки — на отдельной строке, затем пустая строка.
5. Не выводи отдельной строкой дату под заголовком.
6. Грамотность безупречная.
...
${hurricaneGuidance}
Землетрясения опиши только из массива earthquakes: магнитуда, место, локальное время события.
